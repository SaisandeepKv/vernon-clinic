...
function AnimatedIcon({ path, isInView, delay }: { path: string; isInView: boolean; delay: number }) {
  return (
...
function StepCard({ step, index }: { step: typeof steps[number]; index: number }) {
  ...
  return (
    <motion.div
      ref={ref}
      initial={{ opacity: 0, y: 40, rotateZ: index % 2 === 0 ? -2 : 2 }}
    ...
    >
      ...
      <div className="relative flex items-start gap-4 lg:flex-col lg:items-center">
        {/* Number circle with animated icon */}
        <motion.div
          initial={{ scale: 0 }}
