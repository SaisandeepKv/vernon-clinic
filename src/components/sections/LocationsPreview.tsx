  ...
  return (
    ...
    >
      <Link
        ...
        className="group card card-lift flex h-full flex-col bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-vernon-900/5"
      >
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xs font-medium text-clinical-600">
              {detail.tagline}
            </span>
            <h3 className="mt-1 text-xl font-medium text-vernon-900 transition-colors group-hover:text-clinical-700">
              {location.name}
            </h3>
          </div>
          <svg className="h-5 w-5 text-vernon-300 transition-transform group-hover:translate-x-1 group-hover:text-clinical-600" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
          </svg>
        </div>

        <p className="mt-3 flex-1 text-sm text-vernon-500">
          {detail.description}
        </p>

        <div className="mt-4 flex flex-wrap gap-2">
          {detail.highlights.map((h) => (
            <span key={h} className="rounded-full bg-vernon-50 px-2.5 py-0.5 text-xs text-vernon-600 transition-colors group-hover:bg-clinical-50 group-hover:text-clinical-700">
              {h}
            </span>
          ))}
        </div>

        <div className="mt-4 border-t border-vernon-50 pt-4">
          <p className="text-xs text-vernon-400 line-clamp-2">
            {location.address}
          </p>
          <div className="mt-2 flex items-center gap-4">
            {location.timings.map((t) => (
              <span key={t.days} className="text-xs text-vernon-500">
                {t.days}: <span className="text-vernon-700">{t.hours}</span>
              </span>
            ))}
          </div>
        </div>
...
export function LocationsPreview() {
  ...
  return (
    <section className="border-t border-vernon-100 bg-vernon-50 py-20 lg:py-28">
