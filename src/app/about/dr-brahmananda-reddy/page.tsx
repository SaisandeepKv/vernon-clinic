              ...
              <StaggerReveal className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3" staggerDelay={0.12}>
                {doctor.awards.map((award) => (
                  <StaggerItem key={award.title}>
                    <div className="rounded-xl bg-vernon-50 p-4">
                      <p className="text-2xl font-light text-vernon-900">{award.year}</p>
                      <p className="mt-1 text-sm font-medium text-vernon-800">{award.title}</p>
                      <p className="mt-0.5 text-xs text-vernon-400">{award.issuer}</p>
                    </div>
                  </StaggerItem>
                ))}
              </StaggerReveal>
