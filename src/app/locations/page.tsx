          ...
          <StaggerReveal className="mt-12 grid grid-cols-1 gap-8 lg:grid-cols-3" staggerDelay={0.15} baseDelay={0.1}>
            {locations.map((location) => (
              <StaggerItem key={location.id}>
                ...
                >
                  {/* Map placeholder */}
                  <div className="aspect-video overflow-hidden rounded-lg bg-vernon-100">
                    <div className="flex h-full items-center justify-center text-sm text-vernon-400">
                  ...
                  <h2 className="mt-4 text-xl font-medium text-vernon-900 group-hover:text-clinical-700">
                  ...
                  <div className="mt-4 space-y-2">
                    {location.timings.map((t) => (
                      <div key={t.days} className="flex justify-between text-sm">
                        <span className="text-vernon-400">{t.days}</span>
                        <span className="font-medium text-vernon-700">{t.hours}</span>
                      </div>
                    ))}
                  </div>

                  <div className="mt-4 flex flex-wrap gap-2 border-t border-vernon-50 pt-4">
